...
<div class="container">
    <h1>Welcome to SvelteKit DB Example</h1>
    ...
    <form method="post" action="?/{action}">
        ...
        <div>
            <input name="nome" id="nome" type="text" size="40" bind:value={nome}>
            <p class={error && nome?.length == 0? '': 'hidden'}>Nome Invalido</p>
        </div>
        <div><label for="cognome">Cognome</label></div>
        <div>
            <input name="cognome" id="cognome" type="text" size="40" bind:value={cognome}>
            <p class={error && cognome?.length == 0? '': 'hidden'}>Cognome Invalido</p>
        </div>
        <div><label for="eta">Età</label></div>
        <div>
            <input name="eta" id="eta" type="number" min="15" bind:value={eta}>
